...
$(document).ready(() => {
  $('.frame').hover(hoverIn, hoverOut).click(openCanvas); // show/hide frame buttons
    ...
    return (
      <div className="frame-box">
        <ul id="frame-list" className="frame-list">
          <li
            className="frame activeFrame"
            id="frame1"
          >
            <p id="preview-box" className="preview-box"></p>
            <title className="number">1</title>
            <span className="copyFrame hidden"><i className="fas fa-copy"></i></span>
            <span className="removeFrame hidden"><i className="fas fa-trash-alt"></i></span>
          </li>
        </ul>
        <button className="addNewFrame" onClick={addNewFrame}>Add new frame</button>
      </div>
    )
  }
}
